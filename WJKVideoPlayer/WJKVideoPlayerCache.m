...
@implementation WJKVideoPlayerCacheConfiguration
...
static NSString *kWJKVideoPlayerVersion2CacheHasBeenClearedKey = @"com.wujike.version2.cache.clear.key.www";
@implementation WJKVideoPlayerCache

- (instancetype)initWithCacheConfiguration:(WJKVideoPlayerCacheConfiguration *_Nullable)cacheConfiguration {
    self = [super init];
    if (self) {
        // Create IO serial queue
        _ioQueue = dispatch_queue_create("com.Wujike.WJKVideoPlayerCache", DISPATCH_QUEUE_SERIAL);
        pthread_mutexattr_t mutexattr;
        pthread_mutexattr_init(&mutexattr);
        pthread_mutexattr_settype(&mutexattr, PTHREAD_MUTEX_RECURSIVE);
        pthread_mutex_init(&_lock, &mutexattr);
        WJKVideoPlayerCacheConfiguration *configuration = cacheConfiguration;
        if (!configuration) {
            configuration = [[WJKVideoPlayerCacheConfiguration alloc] init];
        }
...
- (void)diskVideoExistsWithKey:(NSString *)key
                    completion:(WJKVideoPlayerCheckCacheCompletion)completion {
    dispatch_async(_ioQueue, ^{
        BOOL exists = [self.fileManager fileExistsAtPath:[WJKVideoPlayerCachePath videoCachePathForKey:key]];
        if (completion) {
...
- (void)queryCacheOperationForKey:(NSString *)key
    ...
    if (!key) {
        if (completion) {
            WJKDispatchSyncOnMainQueue(^{
                completion(nil, WJKVideoPlayerCacheTypeNone);
            });
        }
        return;
    }
    ...
    dispatch_async(self.ioQueue, ^{
        @autoreleasepool {
            ...
            if (completion) {
                WJKDispatchSyncOnMainQueue(^{
                    completion([WJKVideoPlayerCachePath videoCachePathForKey:key], WJKVideoPlayerCacheTypeExisted);
                });
            }
        }
    });
...
- (void)removeVideoCacheForKey:(NSString *)key
    ...
    dispatch_async(self.ioQueue, ^{
        if ([self.fileManager fileExistsAtPath:[WJKVideoPlayerCachePath videoCachePathForKey:key]]) {
            [self.fileManager removeItemAtPath:[WJKVideoPlayerCachePath videoCachePathForKey:key] error:nil];
            [self.fileManager removeItemAtPath:[WJKVideoPlayerCachePath videoCacheIndexFilePathForKey:key] error:nil];
            WJKDispatchSyncOnMainQueue(^{
                if (completion) {
                    completion();
                }
...
- (void)deleteOldFilesOnCompletion:(dispatch_block_t _Nullable)completion {
    dispatch_async(self.ioQueue, ^{
        NSURL *diskCacheURL = [NSURL fileURLWithPath:[WJKVideoPlayerCachePath videoCachePath] isDirectory:YES];
...
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
        [self.fileManager removeItemAtPath:[WJKVideoPlayerCachePath videoCachePathForAllFullFile] error:nil];
        [self.fileManager removeItemAtPath:[WJKVideoPlayerCachePath videoCachePathForAllTemporaryFile] error:nil];
#pragma clang diagnostic pop
        [self.fileManager removeItemAtPath:[WJKVideoPlayerCachePath videoCachePath] error:nil];
        WJKDispatchSyncOnMainQueue(^{
            if (completion) {
                completion();
            }
...
- (void)clearVideoCacheOnVersion2OnCompletion:(dispatch_block_t _Nullable)completion {
    BOOL version2CacheHasBeenCleared = [NSUserDefaults.standardUserDefaults boolForKey:kWJKVideoPlayerVersion2CacheHasBeenClearedKey];
    ...
    [NSUserDefaults.standardUserDefaults setBool:YES forKey:kWJKVideoPlayerVersion2CacheHasBeenClearedKey];
...
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
    NSString *tempFilePath = [WJKVideoPlayerCachePath videoCachePathForAllTemporaryFile];
    NSString *fullFilePath = [WJKVideoPlayerCachePath videoCachePathForAllFullFile];
#pragma clang diagnostic pop
    NSString *videoCachePath = [WJKVideoPlayerCachePath videoCachePath];
    @autoreleasepool {
...
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
    ...
    NSString *fullFilePath = [WJKVideoPlayerCachePath videoCachePathForAllFullFile];
#pragma clang diagnostic pop
    NSString *videoFilePath = [WJKVideoPlayerCachePath videoCachePath];
...
@implementation WJKVideoPlayerCache(Deprecated)

- (void)removeFullCacheForKey:(NSString *)key
                   completion:(dispatch_block_t _Nullable)completion{
    dispatch_async(self.ioQueue, ^{
        if ([self.fileManager fileExistsAtPath:[WJKVideoPlayerCachePath videoCacheFullPathForKey:key]]) {
            [self.fileManager removeItemAtPath:[WJKVideoPlayerCachePath videoCacheFullPathForKey:key] error:nil];
            WJKDispatchSyncOnMainQueue(^{
                if (completion) {
                    completion();
                }
...
- (void)removeTempCacheForKey:(NSString *)key
    ...
    dispatch_async(self.ioQueue, ^{
        NSString *path = [WJKVideoPlayerCachePath videoCachePathForAllTemporaryFile];
