...
@interface WJKVideoPlayerCacheFile()
...
static const NSString *kWJKVideoPlayerCacheFileZoneKey = @"com.wujike.zone.key.www";
static const NSString *kWJKVideoPlayerCacheFileSizeKey = @"com.wujike.size.key.www";
static const NSString *kWJKVideoPlayerCacheFileResponseHeadersKey = @"com.wujike.response.header.key.www";
@implementation WJKVideoPlayerCacheFile
...
- (void)mergeRangesIfNeed {
    ...
    for (int i = 0; i < self.internalFragmentRanges.count; ++i) {
        if ((i + 1) < self.internalFragmentRanges.count) {
            ...
            if (WJKRangeCanMerge(currentRange, nextRange)) {
...
- (BOOL)truncateFileWithFileLength:(NSUInteger)fileLength {
    WJKDebugLog(@"Truncate file to length: %u", fileLength);
    ...
    @catch (NSException * e) {
        WJKErrorLog(@"Truncate file raise a exception: %@", e);
        if (!lock) {
            pthread_mutex_unlock(&_lock);
        }
        return NO;
...
- (BOOL)storeResponse:(NSHTTPURLResponse *)response {
    ...
    if (![self isFileLengthValid]) {
        success = [self truncateFileWithFileLength:(NSUInteger)response.wjk_fileLength];
    }
...
- (void)storeVideoData:(NSData *)data
    ...
    @try {
        ...
        [self.writeFileHandle wjk_safeWriteData:data];
    }
    @catch (NSException * e) {
        WJKErrorLog(@"Write file raise a exception: %@", e);
    }
...
- (BOOL)serializeIndex:(NSDictionary *)indexDictionary {
    ...
    int lock = pthread_mutex_trylock(&_lock);
    NSNumber *fileSize = indexDictionary[kWJKVideoPlayerCacheFileSizeKey];
    ...
    NSMutableArray *rangeArray = indexDictionary[kWJKVideoPlayerCacheFileZoneKey];
    for (NSString *rangeStr in rangeArray) {
        NSRange range = NSRangeFromString(rangeStr);
        [self.internalFragmentRanges addObject:[NSValue valueWithRange:range]];
    }
    self.responseHeaders = indexDictionary[kWJKVideoPlayerCacheFileResponseHeadersKey];
    if (!lock) {
        pthread_mutex_unlock(&_lock);
    }
    return YES;
